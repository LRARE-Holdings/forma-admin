 ...
 *
 * Retries Stripe product/price sync for any products that exist in the DB
 * but are missing their stripe_price_id. Runs periodically via Vercel Cron.
 *
 * Only processes studios that have completed Stripe onboarding.
 */
export async function GET(request: Request) {
  ...
  // Get all studios with completed Stripe onboarding
  ...
  for (const studio of studios) {
    ...
    for (const tier of packTiers ?? []) {
      try {
        ...
        console.error(`[cron/stripe-sync] Failed to sync pack tier ${tier.id}:`, e)
        failed++
      }
    }
    ...
    for (const cls of classes ?? []) {
      try {
        ...
        console.error(`[cron/stripe-sync] Failed to sync class ${cls.id}:`, e)
        failed++
      }
    }
    ...
    for (const tier of membershipTiers ?? []) {
      try {
        ...
        console.error(`[cron/stripe-sync] Failed to sync membership tier ${tier.id}:`, e)
        failed++
      }
    }
  }
